  ...
  <div class="app-init">
    ...
    >
      <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
        <div class="list" v-for="item in store" :key="item.id" >
...
export default {
  ...
  methods: {
    ...
    clicks () {
       if (window._czc) {
               window._czc.push(['_trackEvent', '提现明细', '提现明细时间筛选','' ,'' , '']);//其中灰色的为选填项
         }
    ...
    getList () {
      if (this.kaiguan == true) {
        ...
        this.shopApi.getMoneyList(datejson2).then(res => {
