  ...
  <div class="app-init">
    ...
    <van-collapse v-model="activeNames" class="margintop">
      <van-collapse-item name="0" title="审核信息" value-class="collapsewarn">
        ...
        <van-cell title="审核名称" :value="shop.name" />
    ...
    <van-collapse v-model="activeNames" class="margintop">
      <van-collapse-item name="2" title="商户老板证照信息" value-class="collapsewarn">
        ...
        <van-cell title="证照" value-class="teadd">
          <one-not-cut
            class="notclickadd"
            :one-not-cut="approvalContent.storeAccount==null?'':approvalContent.storeAccount.idCardBackUrl"
          ></one-not-cut>
          <one-not-cut
            class="notclickadd"
            :one-not-cut="approvalContent.storeAccount==null?'':approvalContent.storeAccount.idCardJustUrl"
          ></one-not-cut>
        </van-cell>
      </van-collapse-item>
    </van-collapse>
    <van-collapse v-model="activeNames" class="margintop">
      <van-collapse-item name="3" title="状态信息" value-class="collapsewarn">
        <div slot="icon" class="vant-collapse-icon"></div>
        <!--<div slot="title">状态信息<van-icon name="question-o" /></div>-->
        <div slot="value">
          <van-icon name="info-o" v-show="activewarn.onoff0" />
        ...
        <van-field
          type="text"
          label="推广人"
          :value="approvalContent.store==null?'':approvalContent.store.spreadUsername"
          readonly
        />
        <van-field
          type="text"
        ...
        <!--</van-cell>-->
        <!--<van-cell title="" value-class="teadd" v-if="approvalContentSuccess!=null && change(approvalContent.store==null?'':approvalContent.store.runProvePic,approvalContentSuccess.store==null?'':approvalContentSuccess.store.runProvePic)">-->
    ...
    <van-collapse v-model="activeNames" class="margintop">
      <van-collapse-item title="基本信息" name="4" value=" " value-class="collapsewarn">
        ...
        <van-field
          type="text"
          label="入驻时间"
          :value="format(new Date(approvalContent.store==null?'':approvalContent.store.createTime), 'yyyy-MM-dd hh:mm:ss')"
          readonly
        ></van-field>
        <van-field
          type="text"
        ...
        <van-cell title="店招名称" :value="approvalContent.store==null?'':approvalContent.store.name" />
        <van-field
          type="text"
          label
        ...
        <van-cell title="店铺头像" value-class="teadd">
          <one-not-cut
            class="notclickadd"
            :one-not-cut="approvalContent.store==null?'':approvalContent.store.logo"
          ></one-not-cut>
        </van-cell>
        <van-cell
          title
          value-class="teadd"
          v-if="approvalContentSuccess!=null && change(approvalContent.store==null?'':approvalContent.store.logo,approvalContentSuccess.store==null?'':approvalContentSuccess.store.logo)"
        >
          <p style="color:#f44">变更前：</p>
          <one-not-cut
            :one-not-cut="approvalContentSuccess.store==null?'':approvalContentSuccess.store.logo"
            class="notclickadd"
          ></one-not-cut>
        </van-cell>
        <van-cell
          title="店铺地址"
          v-if="approvalContent.store!=null"
          :value="approvalContent.store.province+approvalContent.store.city+approvalContent.store.region"
        ></van-cell>
        <van-field
          type="text"
          label
          v-if="approvalContentSuccess!=null && approvalContent.store!=null && change(approvalContent.store==null?'':approvalContent.store.region,approvalContentSuccess.store==null?'':approvalContentSuccess.store.region)"
          :value="'变更前：'+(approvalContentSuccess.store.province+approvalContentSuccess.store.city+approvalContentSuccess.store.region)"
          readonly
          error
        />
    ...
    <van-collapse v-model="activeNames" class="margintop">
      <van-collapse-item name="5" title="证照信息" value-class="collapsewarn">
        ...
        <van-cell title="名称" :value="approvalContent.store==null?'':approvalContent.store.name" />
        <van-field
          type="text"
          style="margin-left:12px;"
          v-if="approvalContentSuccess!=null && change(approvalContent.store==null?'':approvalContent.store.name,approvalContentSuccess.store==null?'':approvalContentSuccess.store.name)"
        ...
        <van-field
          type="text"
          style="margin-left:12px;"
          v-if="approvalContentSuccess!=null && change(approvalContent.store==null?'':approvalContent.store.bcEndtime,approvalContentSuccess.store==null?'':approvalContentSuccess.store.bcEndtime)"
        ...
        <van-cell title="证照" value-class="teadd">
          <one-not-cut
            class="notclickadd"
            :one-not-cut="approvalContent.store==null?'':approvalContent.store.bcPicUrl"
          ></one-not-cut>
        </van-cell>
        <van-cell
          title
          value-class="teadd"
          v-if="approvalContentSuccess!=null && change(approvalContent.store==null?'':approvalContent.store.bcPicUrl,approvalContentSuccess.store==null?'':approvalContentSuccess.store.bcPicUrl)"
        >
          <p style="color:#f44">变更前：</p>
          <one-not-cut
            :one-not-cut="approvalContentSuccess.store==null?'':approvalContentSuccess.store.bcPicUrl"
            class="notclickadd"
          ></one-not-cut>
        </van-cell>
        <van-field
        ...
        <van-cell title="法人身份证" value-class="teadd">
          <one-not-cut
            class="notclickadd"
            :one-not-cut="approvalContent.store==null?'':approvalContent.store.idCardBackUrl"
          ></one-not-cut>
          <one-not-cut
            class="notclickadd"
            :one-not-cut="approvalContent.store==null?'':approvalContent.store.idCardJustUrl"
          ></one-not-cut>
        </van-cell>
        <van-cell
          title
          value-class="teadd"
          v-if="approvalContentSuccess!=null
        && ( change(approvalContent.store==null?'':approvalContent.store.idCardBackUrl,approvalContentSuccess.store==null?'':approvalContentSuccess.store.idCardBackUrl)
        || change(approvalContent.store==null?'':approvalContent.store.idCardJustUrl,approvalContentSuccess.store==null?'':approvalContentSuccess.store.idCardJustUrl))"
        >
          <p style="color:#f44">变更前：</p>
          <one-not-cut
            :one-not-cut="approvalContentSuccess.store==null?'':approvalContentSuccess.store.idCardBackUrl"
            class="notclickadd"
          ></one-not-cut>
          <one-not-cut
            :one-not-cut="approvalContentSuccess.store==null?'':approvalContentSuccess.store.idCardJustUrl"
            class="notclickadd"
          ></one-not-cut>
        </van-cell>
        <van-cell title="其他证件" value-class="teadd">
          <one-not-cut
            ...
            :one-not-cut="src"
            :key="index"
            v-for="(src,index) in approvalContent.store==null?'':approvalContent.store.idOthrCerUrl.split(',')"
          ></one-not-cut>
        </van-cell>
        <van-cell
          title
          value-class="teadd"
          v-if="approvalContentSuccess!=null && change(approvalContent.store==null?'':approvalContent.store.idOthrCerUrl,approvalContentSuccess.store==null?'':approvalContentSuccess.store.idOthrCerUrl)"
        >
          <p style="color:#f44">变更前：</p>
          <one-not-cut
            class="notclickadd"
            :one-not-cut="src"
            :key="index"
            v-for="(src,index) in approvalContentSuccess.store==null?'':approvalContentSuccess.store.idOthrCerUrl.split(',')"
          ></one-not-cut>
        </van-cell>
      </van-collapse-item>
    </van-collapse>
    <van-collapse v-model="activeNames" class="margintop">
      <van-collapse-item name="6" title="经营信息" value-class="collapsewarn">
        <div slot="icon" class="vant-collapse-icon"></div>
        <!--<div slot="title">状态信息<van-icon name="question-o" /></div>-->
        <div slot="value">
          <van-icon name="info-o" v-show="activewarn.onoff0" />
        ...
        <!--type="text"-->
        <!--label="店铺分类"-->
        <!--:value="getStoreCategory(approvalContent.storeCategoryInside)"-->
        <!--readonly-->
        <!--/>-->
        <!--<van-field-->
        <!--type="text"-->
        <!--style="margin-left:12px;"-->
        <!--v-if="approvalContentSuccess!=null && approvalContent.storeCategoryInside!=null && change(approvalContent.storeCategoryInside.name,approvalContentSuccess.storeCategoryInside.name)"-->
        <!--:value="'变更前：'+getStoreCategory(approvalContentSuccess.storeCategoryInside)"-->
        <!--readonly-->
        <!--error-->
        <!--/>-->
        <van-field
        ...
        <van-cell title="店铺环境" value-class="teadd">
          <one-not-cut
            class="notclickadd"
            :key="keys"
            v-for="(src,keys) in approvalContent.store==null?'':(approvalContent.store.albums==null?'':approvalContent.store.albums.split(','))"
            :one-not-cut="src"
          ></one-not-cut>
        </van-cell>
        <van-cell
          title
          value-class="teadd"
          v-if="approvalContentSuccess!=null && change(approvalContent.store==null?'':approvalContent.store.albums,approvalContentSuccess.store==null?'':approvalContentSuccess.store.albums)"
        >
          <p style="color:#f44">变更前：</p>
          <one-not-cut
            :key="keys"
            v-for="(src,keys) in approvalContentSuccess.store==null?'':(approvalContentSuccess.store.albums==null?'':approvalContentSuccess.store.albums.split(','))"
            :one-not-cut="src"
            class="notclickadd"
          ></one-not-cut>
        </van-cell>
        <van-field
          type="textarea"
          label="店铺简介"
          :value="approvalContent.store==null?'':approvalContent.store.info"
          readonly
        />
        <van-field
          type="text"
...
import { format, arrToString, accMul } from "@/utils";
export default {
  data () {
    return {
      accMul,
