...
Validator.localize('zh_CN', {
    ...
    messages: {
        ...
        between: (field, [min, max, label]) => `${label || fieldName}必须在${min}与${max}之间`,
...
Validator.extend('telphone', {
    validate: value => {
        return value.length == 11 ||/(\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$/.test(value);
    },
    getMessage: () => '座机格式：区号是3/4，号码是8位,手机号为11位'
})
...
Validator.extend('bccode', {
    ...
    getMessage: () => '营业执照号格式不正确'
})
Validator.extend('emoji', {
    validate: (value, [length, max]) => {
        let regRule = /[^\u4E00-\u9FA5|\d|\a-zA-Z|\-+*/|\r\n\s,.?!，。: '";%￥#@（）_<>？！…—&$=(){}[\]]|/g;
        let newvalue = value.replace(regRule, "")
        if (newvalue !== value) {
            return false
        } else if (value.length < length || value.length > max) {
            return false
        } else {
            return true
        }
    ...
    getMessage: (value, [length, max]) => `请勿输入特殊字符并且长度在${length}和${max}之间`
    /*{
        if(value.length<length||value.length>max){

            return `长度在${length}和${max}之间`
        }else{
            return `请不要输入特殊字符`
        }
...
router.onError(() => {
    Toast('版本更新中,请重新登录')
    window.location.href = Vue.prototype.HTTPLOCAT + '/promoteApp.html?time=' + ((new Date()).getTime());
})
