  ...
  >
    <div class="rightmap">
      ...
      <van-cell-group>
        ...
        <van-cell title="换绑手机号" is-link :value="change.password.phone" @click="change.phone.onoff=!change.phone.onoff;topheader.name='换绑手机号'" />
        <van-cell title="修改邮箱" is-link :value="store.email" @click="change.email.onoff=!change.email.onoff;topheader.name='修改邮箱'" />
      </van-cell-group>
    ...
    <div class="rightmap">
      ...
      <form @submit.prevent="changepassword('changePassword')" data-vv-scope="changePassword">
        <van-cell-group>
          ...
          >
            <van-button slot="button" :loading="pwdcodeBtn.loading" v-show="pwdcodeBtn.sendAuthCode" size="small" type="primary" @click="getnote('1')">发送验证码</van-button>
        ...
        <van-cell-group>
          ...
          <van-field
            ...
            v-validate="'confirmed:confirmation,新密码,确认密码'"
            placeholder="请确认密码"
          >
          </van-field>
        </van-cell-group>
  ...
  methods:{
    ...
    exit(){
      ...
      this.$dialog.confirm({
        ...
        this.Api.outByStore(para).then(() => {
          exitvoice(sessionStorage.getItem('storeId')?sessionStorage.getItem('storeId'):sessionStorage.getItem('id'))
    ...
    getdata(){
      ...
      formdata.set('storeAccountId', sessionStorage.getItem('id'))*/
