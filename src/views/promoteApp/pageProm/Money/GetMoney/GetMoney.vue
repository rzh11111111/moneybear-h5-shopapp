<template>
  <div class="app-init">
    ...
    <p class="getmney-notice">每月18-20号可提现1次，50元起提</p>
...
<script>
import { currency, accMul, accDiv } from "@/utils";
export default {
  name: "GetMoney",
  data () {
    return {
  ...
  methods: {
    ...
    allget () {
      let regRule = /,/g;
      let param = this.canmoney.toString()
    ...
    inputbtn () {
      if (window._czc) {
               window._czc.push(['_trackEvent', '邀店', '确认提现','' ,'' , '']);//其中灰色的为选填项
      
         }
      if (Number(this.money) < 50) {
        this.$toast('50元起提')
      } else if (Number(this.canmoney) < Number(this.money)) {
        this.$toast('可提现余额不足')
      } else {
        let para = {
          extractMoney: accMul(Number(this.money), 100)
        }
        this.Api.togetMoney(para).then(res => {

          console.log(res)
          this.$dialog.confirm({
            message: '提现金额：' + this.money + '元<br>扣除税费：-' + accDiv(res.data.data, 100) + '元<br>实际到账：' + accDiv((accMul(Number(this.money), 100) - res.data.data), 100) + '元'
          }).then(() => {
            this.togetMoneys()
          }).catch(() => {
            // on cancel
    ...
    togetMoneys () {
      ...
      this.Api.getMoney(para).then(res => {
        ...
        if (res.data.status == 1) {
          this.$router.push({ path: '/ApplyResult', query: { result: '提现成功'} })
    ...
    getdata () { // 获取全局数据
      ...
      this.Api.money(para).then(res => {
        ...
        if (res.data.status == 1) {
          ...
          this.store.moneyBrokerageReceivable = currency(accDiv(this.store.moneyBrokerageReceivable, 100))
          this.store.money = currency(accDiv(this.store.money, 100))
        }

      })
      this.Api.canGetMoney(para).then(res => {
        // sessionStorage.$setSessionStorageByName('userId', res.data.userId)
        // setToken(res.data.phone)
        if (res.data.status == 1) {
          if (res.data.data === 0) {
            ...
            this.canmoney = currency(accDiv(res.data.data, 100))
          }
        }
    ...
    getdata2 () {
      ...
      this.Api.canGetMoney2(para).then(res => {
        // sessionStorage.$setSessionStorageByName('userId', res.data.userId)
        // setToken(res.data.phone)
        if (res.data.status == 1) {
          if (res.data.data === 0) {
    ...
    todetail () {
      if (this.type == '3') {
        this.$router.push({ path: '/BossDetail', query: { id: sessionStorage.getItem('id'), type: this.type, fromrouter: '2', lookcard: true } })
      } else if (this.type == '2') {
        this.$router.push({ path: '/PromotersListDetail', query: { id: sessionStorage.getItem('id'), type: this.type, fromrouter: '2', lookcard: true } })
      } else if (this.type == '1') {
        this.$router.push({ path: '/PopularizeDetail', query: { id: sessionStorage.getItem('id'), type: this.type, fromrouter: '2', lookcard: true } })
      } else if (this.type == '0') {
        this.$router.push({ path: '/MasterDetail', query: { id: sessionStorage.getItem('id'), type: this.type, fromrouter: '2', lookcard: true } })
      }
    }
  },
  mounted () {
