<template>
  <div class="app-init">
    ...
    <van-collapse :value="activeNames">
      ...
      <van-cell title="合作商资料变更" is-link @click="toAuditList(27)">
        <div class="graycolor">
            <span class="bluecolor">{{updateCooperationCount}}</span>待审核
            <span class="redcolor">{{updateCooperationRefusedCount}}</span>拒绝
        </div>
      </van-cell>
      <van-cell title="推广员资料变更" is-link @click="toAuditList(28)">
        <div class="graycolor">
          <span class="bluecolor">{{updateSpreadCount}}</span>待审核
          <span class="redcolor">{{updateSpreadRefusedCount}}</span>拒绝
        </div>
      </van-cell>
    </van-collapse>
  </div>
